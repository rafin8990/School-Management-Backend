...
const createCategory = async (data: ICategory): Promise<ICategory | null> => {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO category (name, serial_number, status, school_id)
      VALUES ($1, $2, $3, $4)
      RETURNING *;
    `;

    const values = [data.name, data.serial_number ?? null, data.status, data.school_id];
...
const getAllCategories = async (
  ...
  const countQuery = `SELECT COUNT(*) FROM category c ${whereClause};`;
...
const updateCategory = async (id: number, data: Partial<ICategory>): Promise<ICategory | null> => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
